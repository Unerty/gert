...
export function binomialDistribution(s: number, n: number, p: number) {
  return Math.pow((p * Math.pow(Math.E, s) + 1 - p), n);
}

export function discreteDistribution(s: number, pArray: Array<number>, TArray: Array<number>) {
...
export function exponentialDistribution(s: number, a: number) {
  return Math.pow((1 - (s / a)), -1);
}

export function gammaDistribution(s: number, a: number, b: number) {
  return Math.pow((1 - (s / a)), (-1 * b));
}

export function geometricalDistribution(s: number, p: number) {
  let nominator: number = p * Math.pow(Math.E, s);
  let denominatior: number = (1 - Math.pow(Math.E, s) + p * Math.pow(Math.E, s));
  return (nominator/denominatior);
}

export function negativeBinomialDistribution(s: number, p: number, r: number) {
  return Math.pow(p/(1 - Math.pow(Math.E, s) + p * Math.pow(Math.E, s)), r);
}

export function normalDistribution(s: number, m: number, sigma: number) {
  return Math.pow(Math.E, s * m + (1/2) * s*s * sigma*sigma);
}

export function puassonDistribution(s: number, lambda: number) {
  return Math.pow(Math.E, lambda*(Math.pow(Math.E, s) - 1))
}

export function continuousUniformDistribution(s: number, a: number, b: number) {
